...
void *func_dispatcher_msg()
{
  ...
  if (listen(listensd, BACKLOG) < 0 ) 
  {
	...
	sem_wait(exit_hdisp_msg);
		exit_hmsg = 1;
	sem_post(exit_hdisp_msg);	
...
{	
  ...
  int                len;
    ...
    recv(connsd,buff,sizeof(Gossip_message),0);
		...
		if(_DEBUG)
		{
			stampa_rcv_Gossip_message(buff);
		}
		char c ;
...
void helper_ping(Gossip_message *buff)
{
	...
	{
		if(search_buffer_bye_peer(buff->port , buff->ip , buffer_bye_peer) )
		{
			delete_bye_peer(buff->port , buff->ip , buffer_bye_peer);
		}
		if(buff->header.age == 0) // se ha age 0 lo inserisco e invio il pong
		{
  		...
  		struct sockaddr_in server;
			...
			if(_DEBUG)
			{
				stampa_snd_Gossip_message(&msg_pong , buff->port , buff->ip );
			}
			close(sd);
...
void helper_bye(Gossip_message *buff)
{
	...
	{	
		searchInsert_buffer_bye_peer(buff->port, buff->ip , buffer_bye_peer);
...
{
	if(searchInsert_id_buffer_message(buff->header.id,buffer_message) || search_buffer_bye_peer(buff->port , buff->ip , buffer_bye_peer)  )
 // se il messaggio è presente nel buffer_message o e' il peer è presente nel buffer_bye_peer lo scarto
	{
			free(buff);
	}
	else
	{
        ...
        exit(3);
			...
			if(_DEBUG)
			{
				stampa_snd_Gossip_message(msg_query_hit ,  buff->port , buff->ip);
			}
...
void helper_query_hit(Gossip_message *buff)
{
	...
	{ 	// procedura per l'inserimento del gossip_query_hit nel buffer event 
		lock_Two_rooms(sem_event);
			...
			{
				if(dif_str(buffer_event->evt[i].id , buff->hash_file[0] , _ID_M) == 0 )
				{	
