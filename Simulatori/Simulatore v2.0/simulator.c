...
int main()
{
	...
	for(i = 0 ; i < _NUM_NODI ; ++i) //inizializzazione nodi
	{
		...
		{	
			...
			while( b )
			{
				...
				if( num_rand != array_nodi[i].id_nodo ) // non metto nel buffer il nodo identità
				{
					...
					if(b == 0)
					{
						...
						hash_rand = num_random(  _NUM_RISORSE ) +1;

						if( hash_rand > ( _NUM_RISORSE - _HASH_VICINI))
							hash_rand = ( _NUM_RISORSE -_HASH_VICINI ) +1;

						for( h = 0 ; h < _HASH_VICINI; ++h)
						{
							array_nodi[i].hash_arrivi[j][h] = hash_rand;
							++hash_rand; 
						}
			...
			for( h = 0 ; h < _HASH_VICINI; ++h)
			{
				printf(" |%d| " , array_nodi[i].hash_arrivi[j][h]);
			}
			printf("\n") ;
